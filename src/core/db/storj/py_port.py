...
if mode == 'download_file':
    ...
    try:
        size = len(list_objects(bucket_name=bucket_name, prefix=prefix, suffixes=suffixes))
        print(size)
    except:
        print('false')
    ...
    try:
        filenames = list_objects(bucket_name=bucket_name, prefix=prefix, suffixes=suffixes)
